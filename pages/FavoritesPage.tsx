...
const FavoritesPage: React.FC = () => {
  ...
  const [searchTerm, setSearchTerm] = useState(() => sessionStorage.getItem('fav_search') || '');
  const [selectedIds, setSelectedIds] = useState<Set<number>>(new Set());
  const [isSelectionMode, setIsSelectionMode] = useState(false);
  const [selectedGenre, setSelectedGenre] = useState<number | null>(() => {
    const saved = sessionStorage.getItem('fav_genre');
  ...
  useEffect(() => {
    return () => {
      sessionStorage.setItem('fav_search', searchTerm);
      sessionStorage.setItem('fav_genre', selectedGenre ? selectedGenre.toString() : '');
    };
